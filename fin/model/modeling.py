...
def get_last_time_step(out, text_len):
    """Get time step on text_len."""
    batch_range = tf.range(tf.shape(out)[0])
    indices = tf.stack([batch_range, text_len - 1], axis=1)
    last_output = tf.gather_nd(out, indices)
...
def post_attention(h, attn_vec, d_model, n_head, d_head, dropout, is_training,
                   kernel_initializer, residual=True, pre_ln=False):
...
def multihead_attn(q, k, v, attn_mask, d_model, n_head, d_head, dropout,
                   ...
                   scope='abs_attn', pre_ln=False, reuse=None):
...
def rel_multihead_attn(h, r, r_w_bias, r_r_bias, seg_mat, r_s_bias, seg_embed,
                       ...
                       scope='rel_attn', reuse=None, pre_ln=False):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # post processing
        if pre_ln:
            output = post_attention(inp_h, attn_vec, d_model, n_head, d_head, dropout,
                                    is_training, kernel_initializer, pre_ln=pre_ln)
        else:
...
def two_stream_rel_attn(h, g, r, mems, r_w_bias, r_r_bias, seg_mat, r_s_bias,
                        ...
                        kernel_initializer, scope='rel_attn', pre_ln=False):
    ...
    with tf.variable_scope(scope, reuse=False):
        ...
        # post processing
        if pre_ln:
            output_h = post_attention(inp_h, attn_vec_h, d_model, n_head, d_head, dropout,
                                      is_training, kernel_initializer, pre_ln=pre_ln)
        else:
    ...
    with tf.variable_scope(scope, reuse=True):
        ...
        if pre_ln:
            inp_g = g
            g = tf.contrib.layers.layer_norm(g, begin_norm_axis=-1,
                                             scope='LayerNorm')
        ...
        # post processing
        if pre_ln:
            output_g = post_attention(inp_g, attn_vec_g, d_model, n_head, d_head, dropout,
                                      is_training, kernel_initializer, pre_ln=pre_ln)
        else:
...
def transformer_xl(inp_k, n_token, n_layer, d_model, n_head,
    ...
    with tf.variable_scope(scope):
        ...
        if seg_id is not None:
            ...
            mem_pad = tf.zeros([mlen, bsz], dtype=seg_id.dtype)
        ...
        if inp_q is not None:
            output_g = tf.layers.dropout(output_g, dropout, training=is_training)
            output_h = tf.layers.dropout(output_h, dropout, training=is_training)
            if pre_ln:
                output_g = tf.contrib.layers.layer_norm(output_g, begin_norm_axis=-1,
                                                        scope='LayerNorm_output_g')
                output_h = tf.contrib.layers.layer_norm(output_h, begin_norm_axis=-1,
                                                        scope='LayerNorm_output_h')

            return output_h, new_mems, lookup_table, output_g
        else:
            output_h = tf.layers.dropout(output_h, dropout, training=is_training)
            if pre_ln:
                output_h = tf.contrib.layers.layer_norm(output_h, begin_norm_axis=-1,
            ...
            return output_h, new_mems, lookup_table, None
...
def summarize_sequence(summary_type, hidden, d_model, n_head, d_head, dropout,
    ...
    """
        Different classification tasks may not share the same parameters
