...
class XLNetConfig(object):
    ...
    The following hyperparameters are defined:
        ...
        pre_act_fn: string, the activate function in pre_net.
    """

    def __init__(self, FLAGS=None, json_path=None, data_dict=None):
        """Constructing an XLNetConfig.
        One of FLAGS or json_path should be provided."""

        assert FLAGS is not None or json_path is not None or data_dict is not None

        self.keys = ["n_layer", "d_model", "n_head", "d_head", "d_inner",
                     "ff_activation", "untie_r", "n_token", "pre_ln", "pos_len"]
    ...
    def init_from_json(self, json_path):
        with tf.gfile.Open(json_path) as f:
            ...
            for key in self.keys:
                if key == "pre_ln":
                    setattr(self, key, json_data.get(key, False))
                else:
...
class RunConfig(object):
    ...
    def __init__(self, is_training, dropout, dropatt,
                 use_tpu=False, use_bfloat16=False, init="normal", init_range=0.1, init_std=0.02,
                 mem_len=None, reuse_len=None, bi_data=False, clamp_len=-1, same_length=False):
        """
        Args:
...
class XLNetModel(object):
    ...
    def __init__(self, xlnet_config, run_config, input_ids, seg_ids, input_mask,
        ...
        with tf.variable_scope("model", reuse=tf.AUTO_REUSE):
            (self.output, self.new_mems, self.lookup_table,
             self.output_g) = modeling.transformer_xl(**tfm_args)
    ...
    def get_sequence_lm_output(self):
        """
        Returns:
            float32 Tensor in shape [len, bsz, d_model]. The last layer hidden
            representation of XLNet.
